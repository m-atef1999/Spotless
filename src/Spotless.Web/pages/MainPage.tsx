...
import { type Spotless_Application_Dtos_Service_ServiceDto as ServiceDto } from '../lib/index';
...
export const MainPage = () => {
    ...
    const [featuredServices, setFeaturedServices] = useState<ServiceDto[]>([]);
    const [isLoadingServices, setIsLoadingServices] = useState(true);

    useEffect(() => {
        const fetchFeaturedServices = async () => {
            try {
                const data = await ServicesService.getApiServicesFeatured({ count: 3 });
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {isLoadingServices ? (
                            ...
                            featuredServices.map((service) => (
                                <div key={service.id} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:border-blue-200 transition-colors">
                                    ...
                                    <button
                                        onClick={() => navigate(user ? `/customer/services` : '/login')}
                                        className="w-full py-2 text-blue-600 font-medium text-sm hover:bg-blue-50 rounded-lg transition-colors flex items-center justify-center gap-2 group"
                                    >
                                        Learn More

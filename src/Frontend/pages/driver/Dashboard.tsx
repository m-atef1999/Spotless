...
export const DriverDashboard: React.FC = () => {
    ...
    const handleCompleteJob = async (orderId: string) => {
        try {
            setActiveJobs(prev => prev.map(job =>
                job.id === orderId ? new OrderDto({ ...job, status: OrderStatus._3 }) : job
            ));
    ...
    // Calculate stats from active jobs (mock calculation for now as we don't have a dedicated stats endpoint)
    // Calculate stats from active jobs
    const completedJobsCount = activeJobs.filter(j => j.status === OrderStatus._3).length; // 3 = Completed
    const activeJobsCount = activeJobs.filter(j => j.status === OrderStatus._2).length; // 2 = In Progress
            ...
            <div className="space-y-8">
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 mb-8">
                    ...
                    <div className="divide-y divide-slate-100 dark:divide-slate-800">
                        {activeJobs.filter(j => j.status === OrderStatus._2).length === 0 ? (
                            <div className="p-6 text-center text-slate-500">
                                You have no active jobs. Accept a job below to get started!
                            </div>
                        ) : (
                            activeJobs.filter(j => j.status === OrderStatus._2).map((job) => (
                                        ...
                                        <div>
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400">
                                                    {job.status === 0 ? 'Pending' : 'Available'}
                                                </span>
                                            </div>
                                            <div className="flex items-center gap-4 mt-2 text-sm text-slate-500 dark:text-slate-400">
                                                <div className="flex items-center gap-1">

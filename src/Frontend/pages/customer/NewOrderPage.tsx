...
import { ServiceDto, CreateOrderDto, CreateOrderItemDto, PaymentMethod } from '../../lib/apiClient';
...
export const NewOrderPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchServices = async () => {
            try {
                const response = await apiClient.services(undefined, 1, 50);
    ...
    const handleSubmit = async () => {
        setIsLoading(true);
        try {
            const orderItems: CreateOrderItemDto[] = selectedServices.map(s => new CreateOrderItemDto({
            ...
            await apiClient.ordersPOST(order);
        ...
        <div className="space-y-8 max-w-md mx-auto">
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <button
                        className={`p-4 rounded-xl border flex flex-col items-center gap-2 transition-all ${paymentMethod === PaymentMethod.CreditCard
                    ...
                    <button
                        className={`p-4 rounded-xl border flex flex-col items-center gap-2 transition-all ${paymentMethod === PaymentMethod.Cash
                            ? 'border-cyan-500 bg-cyan-50 dark:bg-cyan-900/20 text-cyan-700 dark:text-cyan-300'
                            : 'border-slate-200 dark:border-slate-700 hover:border-slate-300'
                            }`}
        ...
        <DashboardLayout role="Customer">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="mb-12 relative">
                    ...
                    <div className="flex justify-between">
                        {STEPS.map((step) => {
                            ...
                            return (
                                <div key={step.id} className="flex flex-col items-center bg-white dark:bg-slate-950 px-4">
                                    <div
                                        className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all ${isCompleted || isCurrent
                                    ...
                                    <span className={`mt-2 text-sm font-medium ${isCurrent ? 'text-cyan-600 dark:text-cyan-400' : 'text-slate-500 dark:text-slate-400'
                                        }`}>
                                        {step.title}
                                    </span>
                                </div>
                ...
                <motion.div
                    ...
                    exit={{ opacity: 0, x: -20 }}
                    className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-6 min-h-[400px]"
                >
